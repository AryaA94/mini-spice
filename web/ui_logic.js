...
// The analysis each preset is meant to show. Clicking a preset switches to
// that tab, sets its parameters and runs it, so the first click already
// shows something meaningful. Transient/AC settings match the dt/stop (or
// sweep) the corresponding examples/*/ outputs were generated with.
...
// Example text for the main value field, per type (was "1k" for everything).
const VALUE_EXAMPLE={R:"1k",C:"1u",L:"10m",V:"5",I:"1m",D:"1e-14",Q:"1e-16",E:"2",G:"1m"};

// Names and nodes go straight into netlist text, so they must be single
// tokens the parser can't misread: no whitespace (which splits a line into
// different fields -- "in put" silently became two nodes), no '*' or '#'
// (which start a comment anywhere on a line), no parentheses or '='.
const NODE_RE=/^[A-Za-z0-9_]+$/;
const NAME_RE=/^[A-Za-z][A-Za-z0-9_]*$/;
const isGround = n => n==="0" || n.toLowerCase()==="gnd";

// Limits that keep a run from freezing the tab: each transient step / AC
// point is a full matrix solve, and the results are all held in memory.
...
// Engineering notation for display: 4700 -> "4.7 k" + unit, -4.1887e-5 A ->
// "-41.89 µA". Outside f..T it falls back to exponent form ("1e-16 A").
...
// The same, but in SPICE's own suffix spelling (u for micro, meg for mega,
// no space) -- used where the text reads as netlist values, e.g. a waveform.
function fmtSpice(v){
  if(v===0) return "0";
  const a=Math.abs(v);
  for(const [f,p] of SI_PREFIXES){
...
// Called after any edit to the circuit: results on screen no longer match
// it, so dim them with a note until the next run.
...
function renderTable(){
  ...
  rows.forEach((r,idx)=>{
    ...
    // Every user-typed string goes through escapeHtml before innerHTML.
    const e=escapeHtml;
...
/* ---- editing an existing row: loads it into the add form, and "+ Add"
   becomes "Save changes", which replaces the row in place ---- */
let editingIndex=-1;
...
function startEdit(idx){
  ...
  editingIndex=idx;
  clearForm();
  // String(v), not a rounded display format, so saving an unchanged row
  // writes back exactly the same number.
...
function addComponentFromForm(){
  ...
  try{
    ...
    // SPICE treats component names case-insensitively, so R1 and r1 would
    // be the same component to anyone reading the netlist. The row being
    ...
    // Node names are case-sensitive in this engine ("out" and "Out" are two
    // different, unconnected nodes), which is almost never what someone
    // typing "Out" meant -- so catch a near-miss against an existing node.
...
// Tick spacing of 1, 2 or 5 x 10^k: the steps a person would draw by hand.
...
// One SI prefix for a whole axis, so ticks read "0 1 2 3" under a
// "time (ms)" title instead of "0 0.001 0.002 0.003".
...
// Chart state for the hover readout, keyed by chart id; rebuilt per render.
...
function makeChart({series, xLabel, xUnit="", yLabel, yUnit="", xLog=false, refLine=null, refLabel="", yPrefixed=true}){
  ...
  // Draw at roughly the on-screen width so text stays readable on phones
  // (a fixed 760-wide drawing shrunk to a 350px screen made 10px text ~4px).
  ...
  // Pad a little, then snap the y range out to whole tick steps.
  ...
  let xTitle;
  if(xLog){
    // Decade ticks labeled 10, 100, 1k, ...; light minor lines at 2..9.
  ...
  if(refLine!==null){
    ...
    // Label sits just below its line: -3 dB is always under the 0 dB
    // passband trace, so above the line it collided with it.
...
// Hover (or touch-drag) readout: a crosshair at the nearest sample, a dot on
// each trace, and a tooltip with the exact values.
...
// Reads one analysis parameter, naming the field in any error.
...
function run(){
  ...
  try{
    if(mode==="dc"){
      ...
      // The engine takes an integer here; 2.5 used to be silently truncated to 2.
      ...
      // Without an AC excitation every node is exactly 0, which used to plot
      // as a flat -6000 dB line with no explanation.
...
// Enter adds (or saves) from any text field in the add form; Escape cancels
// an edit. Enter in an analysis field runs the simulation.
